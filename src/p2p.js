...
const queryChainLengthMsg = () => ({ 'type': MessageType.LatestQuery, 'data': null });

const queryAllMsg = () => ({ 'type': MessageType.AllQuery, 'data': null });

const responseChainMsg = () => ({
    'type': MessageType.RESBlockchain, 'data': JSON.stringify(blockchain_1.getBlockchain())
});
...
const handleBlockchainResponse = (receivedBlocks) => {
    ...
    if (latestBlockReceived.index > latestBlockHeld.index) {
        ...
        console.log('Warning! Received blockchain is not longer than received blockchain. Do nothing');
    }
};
